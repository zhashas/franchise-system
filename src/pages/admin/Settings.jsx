...
Active page is highlighted in dark/black in the sidebar. Notification badges appear on the Notifications nav item when unread messages exist.`,
  ...
  },
  {
    ...
    summary: "Adding, editing, and managing staff accounts and permissions.",
    articles: [
      {
        title: "Adding a Staff Member",
        content: `Only administrators can create staff accounts.

Steps:
1. Navigate to Manage Staff in the sidebar
2. Click "Add Staff Member"
3. Enter the staff member's full name and email address
4. A temporary password will be assigned or the user will receive a setup email
5. The staff member's role is set to "staff" automatically in the profiles table

Staff accounts have access to: Applications, Appointments, Reports, Notifications, and Account Settings. They cannot access Manage Staff, Add Applicant, Activity Logs, or system-level settings.`,
      },
      {
        title: "Managing Existing Staff",
        content: `Administrators can view all staff accounts from the Manage Staff page.

Available actions:
• View profile — Name, email, role, and account creation date
• Deactivate — Prevents login without deleting the account
• Delete — Permanently removes the staff account and all associated data

Note: Deleting a staff account will cascade-nullify their activity log entries per the ON DELETE SET NULL constraint in the database schema.`,
      },
    ],
  },
  {
...
5. rejected — Application did not meet requirements (admin_remarks required)

Status transitions are one-directional in normal workflow. Only admins can change status from for_release back to approved if needed.`,
      },
      {
...
5. Add admin_remarks if rejecting
6. Update the status using the status dropdown

All status changes are recorded in the activity_logs table with the staff/admin ID, timestamp, and details.`,
      },
      {
...
All reports can be exported to PDF or CSV format using the export buttons in each report section.`,
      },
      {
        ...
        content: `The admin dashboard displays key metrics at a glance:
...
The activity feed shows the latest 10 system events in real time. Click any entry to view full details. The franchise expiry chart shows a monthly breakdown of upcoming expirations.`,
      },
    ],
  },
  {
...
• staff_added / staff_removed — Staff management actions
...
Each log entry records: user_id, user_name, user_email, role, action, details, metadata (JSON), created_at, and ip_address.`,
      ...
      {
        title: "Log Retention & Access",
        content: `Activity logs are immutable — there is no UPDATE policy on the activity_logs table. Only admins can delete log entries, and only through the database directly (not via the UI).

Access levels:
• Admin — Full read access to all logs
• Staff — Read access to logs (view only)
• Applicant — No access

Logs are indexed by role, action, and created_at for fast filtering. Use the search and date-range filters in the Activity Logs page to narrow results. For compliance purposes, do not delete logs without proper authorization.`,
      },
    ],
  },
  {
    ...
    tag: "SECURITY",
    title: "Security & RLS",
    summary: "Row-level security policies, password rules, and best practices.",
    articles: [
      {
        title: "Row-Level Security (RLS)",
        content: `The database uses Supabase Row-Level Security (RLS) to enforce data access at the database layer — not just the application layer.

Key policies:
• Profiles — Users can only read/update their own profile. Admins can read all profiles.
• Applications — Applicants see only their own. Admin/staff see all.
• Notifications — Applicants see only their own. Admin/staff see all admin-type notifications.
• Activity Logs — No update policy (immutable). Admin/staff can read. Only user_id match can insert.
• Reports — Admin/staff can insert and update. Applicants can only read their own.

The get_my_role() function is used throughout RLS policies to determine the authenticated user's role without exposing the profiles table directly.`,
      },
      {
...
• Enable 2FA if available in your organization's Supabase plan
...
Q: The database sync card shows "Unreachable".
A: This indicates a Supabase connectivity issue. Check your internet connection or verify that the Supabase project is active and not paused due to inactivity.`,
      },
      {
                            ...
                            {lines.map((line, j) => {
                              if (line.startsWith("•")) {
                                return (
                                  ...
                                  >
                                    <span
                                      className={`mt-1 w-1.5 h-1.5 rounded-full flex-shrink-0 ${activeSection.color.replace("text-", "bg-")}`}
                                    />
