...
export default function ViewApplication() {
  ...
  const handleUpdateStatus = async (status) => {
    ...
    try {
      ...
      // 2. Send notification with CORRECT schema fields
      const { error: notifError } = await supabase
        .from("notifications")
        .insert({
          recipient_id: application.applicant_id, // ✅ Correct column name
          recipient_type: "applicant", // ✅ Required NOT NULL
          sender_id: null, // ✅ Nullable (system notification)
          sender_type: "admin", // ✅ Required NOT NULL
          application_id: application.id, // ✅ FK to applications
          notification_type:
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Review and verify the application below
            </p>
          </div>
          <span
            className={`px-3 py-1.5 rounded-full text-sm font-semibold capitalize ${statusColor(application?.status)}`}
          >
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border-t-4 border-orange-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
              {[
                ...
                {
                  label: "Email",
                  value: d.email || application?.profiles?.email,
                },
                {
                  label: "Contact Number",
                  value: d.contact_number || application?.profiles?.phone,
                },
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border-t-4 border-blue-500">
                ...
                <div className="flex gap-4">
                  <button
                    ...
                    disabled={updating}
                    className="flex-1 bg-green-500 hover:bg-green-600 text-white py-3 rounded-lg font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    ✅ Approve Application
                  </button>
                  <button
                    ...
                    disabled={updating}
                    className="flex-1 bg-red-500 hover:bg-red-600 text-white py-3 rounded-lg font-semibold transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    ❌ Decline Application
                  </button>
                </div>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm border-t-4 border-orange-500">
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={updating}
                className={`flex-1 text-white py-2 rounded-lg font-semibold text-sm transition disabled:opacity-50 disabled:cursor-not-allowed ${
                  showConfirm === "approved"
              ...
              <button
                ...
                disabled={updating}
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 py-2 rounded-lg font-semibold text-sm transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
            </div>
